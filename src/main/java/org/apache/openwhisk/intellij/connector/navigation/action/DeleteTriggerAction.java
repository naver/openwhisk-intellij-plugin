...
import org.apache.openwhisk.intellij.connector.dialog.trigger.DeleteTriggerDialog;
...
public class DeleteTriggerAction extends AnAction {
    private final static Logger LOG = Logger.getInstance(DeleteTriggerAction.class);
        ...
        if (whiskAuth != null && whiskTriggerMetaData != null) {
            if ((new DeleteTriggerDialog(e.getProject(), whiskTriggerMetaData)).showAndGet()) {
                try {
                    ...
                    ActionManager.getInstance().getAction("WhiskNavigator.Actions.Navigation.Controls.Refresh").actionPerformed(e);
